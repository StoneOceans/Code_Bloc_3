...
import { Box, Button, Heading, Text, VStack, Grid, Spinner, Divider } from "@chakra-ui/react";
...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get("http://sitedesjo.college-hanned/api/orders/", { withCredentials: true });
        ...
        {loading ? (
          ...
          <Grid templateColumns="repeat(auto-fit, minmax(300px, 1fr))" gap={6} w="100%">
            {orders.map((order, index) => (
              <Box key={`${order.id}-${index}`} p={6} bg="white" borderWidth="1px" borderRadius="md" boxShadow="lg">
